...
/**
 * Cette classe permet de modeliser un plan
 *
 */
public class Plan extends Objet{
	 ...
	 */
	public Plan(Matiere m, Point3D p0, Point3D p1, Point3D p2) throws NormeIncorrectException{
		...
		this.P1 = p1;
		this.P2 = p2;
        Vecteur3D v1 = new Vecteur3D(P0, P1);
        Vecteur3D v2 = new Vecteur3D(P0, P2);
        if (v1.getNorme()<=0 || v2.getNorme()<=0){
            throw new NormeIncorrectException(MessageErreur.MSG_ERREUR_NORM_NOT_POS);
        }
        
        v1.normaliser();
        v2.normaliser();
        
        if (v1.getProduitScalaire(v2)==v1.getNorme()*v2.getNorme()){
            throw new NormeIncorrectException(MessageErreur.MSG_ERREUR_POINT_NOT_COLIN);
        }
	}
